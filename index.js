...
var _ = require('lodash'),
  ...
  str2json = require('../string-to-json/lib/string-to-json');
  ...
  return function (req, res, next) {
        ...
        req.on('data', function(chunk) {
          ...
          if (body.substring(0,1) != "{" ){
            ...
            for(var attributeName in obj){
              postUrl = postUrl + attributeName+"="+obj[attributeName] + "&";              
            }
    ...
    function doProcess() {
        ...
        traverseHelpers.findQuerySubdirectory(concatUrl(options.pathRoot, url), query, function (err, subdir) {
          ...
          if (subdir && fs.statSync(concatUrl(options.pathRoot, url, subdir))) {
            mockRequest(concatUrl(options.pathRoot, url, subdir, method));
            util.log("File Path : " + concatUrl(options.pathRoot, url, subdir));            
          } else {
            mockRequest(concatUrl(options.pathRoot, url, method));
            util.log("File Path : " + concatUrl(options.pathRoot, url));
          }
        });
    }
